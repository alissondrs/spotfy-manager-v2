...
class BaseConfig:
    ...
    def __init__(self, service_name: str):
        ...
        self.spotify_auth_url = os.getenv("SPOTIFY_AUTH_URL", "https://accounts.spotify.com/authorize").rstrip("/")
        self.spotify_token_url = os.getenv("SPOTIFY_TOKEN_URL", "https://accounts.spotify.com/api/token").rstrip("/")
        self.spotify_api_url = os.getenv("SPOTIFY_API_URL", "https://api.spotify.com").rstrip("/")
        self.spotify_redirect_uri = os.getenv(
            "SPOTIFY_REDIRECT_URI", "http://127.0.0.1:8000/api/auth/spotify/callback").rstrip("/")
        self.spotify_scopes = os.getenv(
            "SPOTIFY_SCOPES", "user-library-read playlist-read-private playlist-read-collaborative")
        self.session_idle_ttl_seconds = int(os.getenv("WEB_SESSION_IDLE_TTL_SECONDS", "2592000"))
        self.session_absolute_ttl_seconds = int(os.getenv("WEB_SESSION_ABSOLUTE_TTL_SECONDS", "15552000"))
        self.oauth_state_ttl_seconds = int(os.getenv("WEB_OAUTH_STATE_TTL_SECONDS", "600"))
        self.session_cookie_secure = os.getenv("WEB_SESSION_COOKIE_SECURE", "0") in ("1", "true", "yes")
        self.session_cookie_samesite = os.getenv("WEB_SESSION_COOKIE_SAMESITE", "lax").lower()
        self.web_token_encryption_key = os.getenv("WEB_TOKEN_ENCRYPTION_KEY", "")
        self.web_internal_jwt_ttl_minutes = int(os.getenv("WEB_INTERNAL_JWT_TTL_MINUTES", "15"))
